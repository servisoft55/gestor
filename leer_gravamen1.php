...
<body>
		<div id="cuerpo">
<?php 
//$_SESSION['usuario'] = "rsilva";    //  Activar esta opción cuando se incorpore al sistema de inscripciones (con Inicio de Sesion)
//$usuario = $_SESSION['usuario'] ;    //  Activar esta opción cuando se incorpore al sistema de inscripciones (con Inicio de Sesion)

$usuario = $_GET['usuario'];    //  Variable temporal para ejecutar programa en forma individual (SIN INICIAR SESION)
if ($usuario=="gmanriquez") {
	$usuario = "rsilva";	
	$usuario1 = "gmc";	
	} else {
...
//	$texto = mb_convert_encoding($texto, "CP852", "UTF-8");
	$texto_final = iconv('CP850', 'utf8//TRANSLIT', $texto);
	return $texto_final;
}
...
$archivo=fopen("rsilva2.txt", "r") or die("Problemas al abrir el Archivo1");
//$archivo=fopen("y:\\rsilva\\rsilva1.txt", "r") or die("Problemas al abrir el Archivo1");
//$archivo=fopen("\\\\192.168.0.252\\inscripciones\\".$usuario."\\".$usuario1."2.txt", "r") or die("Problemas al abrir el Archivo");
//$archivo=fopen("\\\\192.168.0.252\\inscripciones\\".$usuario."\\".$usuario1."2.txt", "r") or die("Problemas al abrir el Archivo");
$x=0;
$texto1="";
while(!feof($archivo)) {
